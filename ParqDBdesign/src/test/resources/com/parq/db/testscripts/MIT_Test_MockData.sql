...
INSERT INTO parkinglocation (client_id, grid_id, location_identifier, location_name, latitude, longitude)  
	VALUE ((SELECT client_id FROM client WHERE name= 'MIT_TEST'), 
	       (SELECT grid_id FROM parkinggrid WHERE grid_name = 'MIT_Campus'),
			'VoGa', 'Volvo_Garage', 42.361985, -71.098958);
...
INSERT INTO parkinglocation (client_id, grid_id, location_identifier, location_name, latitude, longitude)  
	VALUE ((SELECT client_id FROM client WHERE name= 'MIT_TEST'),
			(SELECT grid_id FROM parkinggrid WHERE grid_name = 'MIT_Campus'),
			'AlGa', 'Albany_Garage', 42.362008, -71.092789);
